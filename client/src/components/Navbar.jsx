...
const Navbar = () => {
  ...
  return (
    <div className='Navbar w-full h-2vh text-[16px] font-semibold  text-gray-700 mt-4 '>
        ...
        <ul className=' flex  items-center justify-around border shadow-md px-4 rounded-full   gap-10'>
            <li><a href="#">MEN</a></li>
        ...
        <div className=' flex   items-center justify-center border px-2 gap-1 rounded-full shadow-lg '>
              {/* //setting the name of loggedin user after login. Double !! convert variable into boolean */}
              { !! user && (
                <div className='text-center  text-[12px] px-1 flex justify-center items-center'>
                  {user.name}
                </div>
              )}

            <Link to={'/login'} className='text-lg bg-[#FF395D]  text-white  rounded-full p-1 '> 
              <div>
                  <BsFillPersonFill/>
              </div>
            </Link> 
